...
class MultiViewPoseDataset(PoseDataset):
	def __init__(self, cfg):
		self.cfg = cfg
		assert not cfg.mirror, 'mirror augmentation not yet supported for multiview dataset'
		# assert not hasattr(self.cfg, 'min_input_size') and not hasattr(self.cfg, 'max_input_size'), 'min and max input sizes not yet supported for multiview dataset'
		if cfg.crop:
			print('multiview dataset does not support cropping')
			cfg.crop = False
		self.singleViewDatasets = []
		for dataset in cfg.multiview_datasets:
...
#                print(sample)
	...
	def get_batch(self, index, scale):
		data_item = self.get_training_sample(index)
		return self.make_batch(data_item, scale, False)
	...
	def make_batch(self, data_item, scale, mirror):
		...
		image = imageio.imread(os.path.join(self.cfg.project_path,im_file), pilmode='RGB')
